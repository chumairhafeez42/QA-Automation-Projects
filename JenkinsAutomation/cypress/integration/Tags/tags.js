...
When('User click on Tags', () => {
    tagLocator.navigateTags().click({force : true})
})

// Check create button is enable when Tag form is empty
Given('User is navigate to Tags page', () => {
    let getHeader = tagLocator.tags_breadcrums()
    if (getHeader == "Tags") {
        console.log('Tags heading found.')
    }
    tagLocator.tags_breadcrums().should('be.visible')
})
When('User clicks on New Tag button', () => {
    tagLocator.getNewTag().should('be.exist').click({force: true})
...
Then('Create button is enabled for empty form', () => {
    ...
    tagLocator.getImageDiv().should('be.exist').within(() => {
        tagLocator.tag_image().should('be.exist').click({force : true})
    })
})
...
And('A new Modal will be appears', () => {
    cy.fixture('testdata').then(function (data) {
        this.data = data;
        tagLocator.getNewTagModalAppear().contains(this.data.Add_New_Tag_txt)
    })
})
...
And('User close the modal', () => {
    tagLocator.getImageDiv().within(() => {
        tagLocator.tag_image().click({force : true})
    })
})
...
And('User clicks on Create button', () => {
    tagLocator.getCreateEnablesButton().should('be.exist').click({force:true})
})
...
Then('Newly added tag name is showing in list', () => {
   tagLocator.newtable_row().eq(0).within(checkBox => {
        tagLocator.table_coulmn().click({force : true})
        tagLocator.tag_name().eq(0).invoke('text').then(($tagName) => {
...
Then('Create button disabled', () => {
    ...
    cy.fixture('testdata').then(function (data) {
        this.data = data;
        tagLocator.tagnamevalidation().contains(this.data.tag_name_validation)
    })
})
And('User close the modal', () => {
    tagLocator.getCancelButton().click({force : true})
})
...
When('User clicks on Filter dropdown menu', () => {
    tagLocator.AllTagDropDown().click({force : true})
...
Then('Tags added 1 day before are displaying', () => {
    tagLocator.tag_selector().scrollIntoView()
...
Then('Tag will be appeared according to searh condition', () => {
    ...
    tagLocator.EmptyTableRows().within(mytext => {
       tagLocator.tag_description_edit().eq(0).then(($tagName) => {
...
When('User type tag name in the searchbox', () => {
    cy.fixture('testdata').then(function (data) {
        this.data = data;
        tagLocator.SearchTag().type(this.data.search_tagname)
    })
})
And('User types special characters', () => {
    cy.fixture('testdata').then(function (data) {
        this.data = data;
        tagLocator.SearchTag().type(this.data.tag_special_char)
    })
})
...
When('User clicks on pagination button', () => {
    tagLocator.TagPage2().click({force : true})
})
...
When('User changes the values from item containing dropdown list', () => {
    tagLocator.PageDataDropdownds().click({force : true})
})
...
Then('All tags selected', () => {
    tagLocator.AllTagSelector().first().should('be.checked')
})
And('New Tag button disappears', () => {
    cy.wait(2000)
...
And('User Click on Edit', () => {
    tagLocator.editOption().should('be.exist').click({force : true})
    cy.wait(2000)
})
Then('A modal form will appears containing values of selected tag', () => {
    ...
    tagLocator.getImageDiv().within(() => {
        tagLocator.tag_image().should('be.exist').click()
    })
})
...
When('User selects tag by clicking on checkbox', () => {
    tagLocator.tagtablecoulmn().then($el => {
        tagLocator.tagtablerows().eq(1).click()
        tagLocator.tag_name().eq(0).then(($tagName) => {
            oldTag = $tagName.text();
...
And('Click on Update button', () => {
    tagLocator.updateButton().click({force : true})
})
...
When('User selects tag by clicking on checkbox', () => {
    tagLocator.tagtablecoulmn().then($el => {
        tagLocator.tagtablerows().eq(1).click({force :true})
    })
})
And('All filter dropdown disappears', () => {
    tagLocator.editOption().scrollIntoView()
...
And('User clicks on delete option', () => {
    tagLocator.deleteOption().eq(1).click({force :true})
})
...
And('User clicks on delete button', () => {
    tagLocator.deleteButton().click({force :true})
})
...
And('User clicks on cancel button', () => {
    tagLocator.DeleteModal().within(checkBox => {
        tagLocator.tag_cancel().eq(0).click()
    })
})
...
And('User clicks on Create button', () => {
    tagLocator.getCreateEnablesButton().click({force :true})
})
And('Success message appears', () => {
    tagLocator.successMessage().should('be.visible')
